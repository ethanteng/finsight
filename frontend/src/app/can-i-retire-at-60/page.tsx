import type { Metadata } from "next";
import StructuredData from "@/components/StructuredData";
import AnswerPage from "@/components/marketing/AnswerPage";
import { buildAnswerPageSchemas, canIRetireAt60 } from "@/lib/answer-pages";

const page = canIRetireAt60;
const canonical = `https://asklinc.com/${page.slug}`;

export const metadata: Metadata = {
  title: "Can I Retire at 60? Savings, Social Security & Medicare",
  description: page.description,
  keywords: [
    "can I retire at 60",
    "how much money do I need to retire at 60",
    "retire at 60",
    "retirement planning at 60",
    "health insurance before Medicare",
    "Social Security at 62",
  ],
  alternates: { canonical },
  openGraph: {
    title: "Can I Retire at 60?",
    description: page.description,
    type: "article",
    url: canonical,
    siteName: "Ask Linc",
    images: [{ url: "https://asklinc.com/og-image.jpg", width: 1200, height: 630, alt: "Ask Linc retirement planning answer for age 60" }],
  },
  twitter: {
    card: "summary_large_image",
    title: "Can I Retire at 60?",
    description: page.description,
    images: ["https://asklinc.com/og-image.jpg"],
  },
  robots: {
    index: true,
    follow: true,
    googleBot: { index: true, follow: true, "max-video-preview": -1, "max-image-preview": "large", "max-snippet": -1 },
  },
};

export default function CanIRetireAt60Page() {
  const schemas = buildAnswerPageSchemas(page);

  return (
    <>
      <StructuredData data={schemas.article} />
      <StructuredData data={schemas.breadcrumbs} />
      <StructuredData data={schemas.faq} />
      <AnswerPage page={page} />
    </>
  );
}
