...
type Breakdown = { key: string; label: string; sessions: number; users: number | null; engagedRate: number | null; ctaRate: number | null; bounceRate?: number | null; pageViewsPerSession?: number | null; engagementSeconds?: number | null; share?: number | null; raw?: Record<string, string> };
...
interface Report {
  ...
  coverage: { eventTrackingStartedAt: string; fullyObservedThrough: string | null; usesFallbackSnapshot: boolean };
  ...
  warnings: string[];
  filterOptions: { sources: string[]; channels: string[]; campaigns: string[]; landingPages: string[]; devices: string[]; visitorTypes: string[]; intents: IntentId[] };
}
...
function Empty() {
  return <div className="rounded-2xl border border-dashed border-[#102319]/15 bg-[#f8f7ef] p-5 text-center"><Lightbulb className="mx-auto text-[#64806e]" size={18} /><p className="mt-2 text-xs leading-5 text-[#66736b]">This breakdown will populate from the GA4 export. No numbers are inferred while the connector is unavailable.</p></div>;
}
...
export default function MarketingDashboardPage() {
  ...
  return <><PageMeta title="Marketing performance | Ask Linc" description="Authenticated acquisition, behavior, intent, and trial conversion diagnostics for Ask Linc." /><div className="authenticated-site min-h-screen bg-[#f2f1e8] text-[#102319]"><AuthenticatedPageHeader activePage="admin" eyebrow="Growth intelligence" title="Marketing performance" /><main className="mx-auto max-w-[1320px] px-4 pb-20 pt-7 sm:px-6">
    ...
    <section className="rounded-b-[22px] border border-t-0 border-[#102319]/10 bg-[#fffdf5] p-4 shadow-[0_16px_40px_rgba(16,35,25,.05)]"><div className="flex flex-wrap items-center gap-2"><div className="mr-1 inline-flex items-center gap-2 text-xs font-extrabold uppercase tracking-[.11em] text-[#486657]"><Filter size={14} /> View</div>{([7, 28, 90] as const).map(days => <button key={days} type="button" onClick={() => update('days', days)} className={`min-h-9 rounded-full px-3 text-xs font-bold ${filters.days === days ? 'bg-[#102319] text-white' : 'bg-[#e9eee5] text-[#486657]'}`}>{days}d</button>)}<label className="ml-1 inline-flex min-h-9 items-center gap-2 rounded-full bg-[#e9eee5] px-3 text-xs font-bold text-[#486657]"><input type="checkbox" checked={filters.compare} onChange={event => update('compare', event.target.checked)} /> Previous period</label>{report && <><FilterSelect label="Source" value={filters.source} options={report.filterOptions.sources} onChange={value => update('source', value)} /><FilterSelect label="Channel" value={filters.channel} options={report.filterOptions.channels} onChange={value => update('channel', value)} /><FilterSelect label="Campaign" value={filters.campaign} options={report.filterOptions.campaigns} onChange={value => update('campaign', value)} /><FilterSelect label="Landing" value={filters.landingPage} options={report.filterOptions.landingPages} onChange={value => update('landingPage', value)} /><FilterSelect label="Device" value={filters.device} options={report.filterOptions.devices} onChange={value => update('device', value)} /><FilterSelect label="Visitor" value={filters.visitorType} options={report.filterOptions.visitorTypes} onChange={value => update('visitorType', value as Filters['visitorType'])} /><FilterSelect label="Intent" value={filters.intent} options={report.filterOptions.intents} format={value => INTENT_LABELS[value as IntentId] || value} onChange={value => update('intent', value as IntentId)} /></>}{hasFilters && <button type="button" onClick={() => setFilters({ days: filters.days, compare: filters.compare })} className="ml-auto text-xs font-bold text-[#8b3027]">Clear filters</button>}</div></section>
    ...
    {report && <div className={loading ? 'pointer-events-none opacity-55 transition-opacity' : 'transition-opacity'}>
      ...
      <section className="mt-10 grid gap-5 lg:grid-cols-[1.15fr_.85fr]"><div className="rounded-[22px] border border-[#102319]/10 bg-[#fffdf5] p-5 shadow-[0_18px_45px_rgba(16,35,25,.05)] sm:p-6"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#102319] text-[#d8ff71]"><Sparkles size={18} /></span><div><p className="text-[10px] font-extrabold uppercase tracking-[.13em] text-[#49725a]">Conversion diagnosis</p><h2 className="text-xl font-semibold tracking-[-.03em]">What deserves attention now</h2></div></div><div className="mt-6 space-y-3">{report.findings.map((finding, index) => <article key={finding.title} className="rounded-2xl border border-[#102319]/10 bg-[#f8f7ef] p-4"><div className="flex gap-4"><span className={`mt-0.5 grid h-7 w-7 shrink-0 place-items-center rounded-full text-xs font-extrabold ${finding.severity === 'critical' ? 'bg-[#8b3027] text-white' : finding.severity === 'warning' ? 'bg-[#f0d99d] text-[#76510f]' : finding.severity === 'opportunity' ? 'bg-[#d8ff71] text-[#102319]' : 'bg-[#dfe9e2] text-[#397052]'}`}>{index + 1}</span><div><h3 className="font-bold">{finding.title}</h3><p className="mt-1 text-sm leading-6 text-[#5e6b63]">{finding.detail}</p><p className="mt-2 flex gap-2 text-xs font-semibold leading-5 text-[#397052]"><ArrowRight size={14} className="mt-0.5 shrink-0" />{finding.action}</p></div></div></article>)}</div></div><div className="space-y-5"><section className="rounded-[22px] border border-[#102319]/10 bg-[#e9eee5] p-5 sm:p-6"><div className="flex items-center gap-3"><Database size={18} /><h2 className="text-lg font-semibold">First-party reality check</h2></div><div className="mt-5 grid grid-cols-2 gap-3"><MiniStat label="Accounts created" value={report.firstParty.accountsCreated} /><MiniStat label="Currently verified" value={report.firstParty.accountsCurrentlyVerified} /><MiniStat label="Reached a login" value={report.firstParty.createdAccountsWithLogin} /><MiniStat label="Asked a question" value={report.firstParty.createdAccountsWithConversation} /><MiniStat label="Subscriptions created" value={report.firstParty.subscriptionsCreated} /><MiniStat label="Currently trialing" value={report.firstParty.currentlyTrialingAccounts} /></div><p className="mt-4 text-[11px] leading-5 text-[#66736b]">{report.firstParty.note}</p></section><Panel icon={<Bot size={18} />} eyebrow="Traffic quality" title="Device mix" subtitle="Unknown devices are isolated, not hidden."><div className="mt-5 space-y-3">{report.devices.map(row => <BarRow key={row.key} row={row} total={report.devices.reduce((sum, item) => sum + item.sessions, 0)} meta={`${percent(row.bounceRate ?? null)} bounce`} />)}</div></Panel></div></section>

      <section className="mt-10 rounded-[24px] border border-[#102319]/10 bg-[#fffdf5] p-5 shadow-[0_18px_45px_rgba(16,35,25,.05)] sm:p-7"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-[10px] font-extrabold uppercase tracking-[.15em] text-[#49725a]">Strict same-session funnel</p><h2 className="mt-1 text-2xl font-semibold tracking-[-.04em]">From promise to first successful login</h2></div><span className="rounded-full bg-[#e9eee5] px-3 py-1.5 text-[11px] font-bold text-[#486657]">Instrumented Sep 9, 2026</span></div><div className="mt-7 grid gap-2 md:grid-cols-6 xl:grid-cols-11">{report.funnel.map((step, index) => <div key={step.event} className="relative min-w-0 rounded-xl border border-[#102319]/10 bg-[#f8f7ef] p-3"><div className="text-[9px] font-extrabold uppercase tracking-[.1em] text-[#748078]">{index + 1}</div><div className="mt-2 text-2xl font-semibold tabular-nums">{number(step.sessions)}</div><div className="mt-1 min-h-8 text-[11px] font-bold leading-4">{step.label}</div><div className="mt-2 text-[10px] text-[#748078]">{step.previousStepRate === null ? step.coverage === 'collecting' ? 'Collecting' : 'Entry' : `${percent(step.previousStepRate)} from prior`}</div>{step.rawEventSessions !== undefined && step.rawEventSessions > (step.sessions || 0) && <div className="mt-2 rounded bg-[#f4ead0] p-1.5 text-[9px] font-bold text-[#76510f]">{step.rawEventSessions} raw · sequence gap</div>}</div>)}</div><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{report.funnelErrors.map(row => <div key={row.event} className="rounded-xl border border-[#b84a3d]/15 bg-[#f8e8e3]/70 p-3"><div className="break-words font-mono text-[10px] text-[#8b3027]">{row.event}</div><div className="mt-2 text-sm font-bold text-[#8b3027]">{number(row.sessions)} sessions · {number(row.events)} events</div><div className="mt-1 text-[10px] text-[#8b3027]/70">{percent(row.rate)} of matching submits</div></div>)}</div></section>

      <section className="mt-10 grid gap-5 lg:grid-cols-2"><Panel icon={<BarChart3 size={18} />} eyebrow="Acquisition" title="Where visits are coming from" subtitle="Raw source and medium stay visible so intent labels remain auditable."><div className="mt-5 space-y-3">{report.acquisition.length ? report.acquisition.map(row => <BarRow key={row.key} row={row} total={report.acquisition.reduce((sum, item) => sum + item.sessions, 0)} meta={`${percent(row.bounceRate ?? null)} bounce`} />) : <Empty />}</div></Panel><Panel icon={<Users size={18} />} eyebrow="Audience" title="New versus returning" subtitle="Returning duration may include internal traffic; use the human segment before acting."><div className="mt-5 space-y-4">{report.visitorTypes.length ? report.visitorTypes.map(row => <div key={row.key} className="rounded-2xl bg-[#f8f7ef] p-4"><div className="flex items-start justify-between"><div><div className="font-bold">{row.label}</div><div className="mt-1 text-xs text-[#66736b]">{percent(row.engagedRate)} engaged · {row.pageViewsPerSession?.toFixed(2) || '—'} pages/session</div></div><div className="text-2xl font-semibold tabular-nums">{number(row.sessions)}</div></div><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-[#dfe5dc]"><div className="h-full rounded-full bg-[#397052]" style={{ width: `${Math.max(2, (row.share || 0) * 100)}%` }} /></div></div>) : <Empty />}</div></Panel></section>
