...
export default function MarketingHome({ pricing = FALLBACK_PRICING }: { pricing?: Pricing }) {
  return (
    <main className="marketing-site story-home">
      ...
      <section className="story-flow-section shell" aria-label="From your accounts to your next move"><PlanningFlow /></section>
      <div className="story-narrative shell" id="how-it-works">
        <section className="story-chapter" aria-labelledby="connect-title">
          <div className="story-chapter-copy"><p className="section-kicker">01 / BRING IT TOGETHER</p><h2 id="connect-title">Connect your<br />financial life.</h2><p>Your bank accounts, investments, home, and loans belong in the same picture. Connect your accounts or add details yourself. Linc brings the relevant numbers together.</p><Link className="text-link" href="/integrations">See what you can connect <ArrowRight size={16} aria-hidden="true" /></Link></div>
          <ConnectedLifeVisual />
        </section>
        ...
        <section className="story-chapter" aria-labelledby="change-title">
          <div className="story-chapter-copy"><p className="section-kicker">03 / EXPLORE THE WHAT-IFS</p><h2 id="change-title">Change the assumption,<br /><em>not the spreadsheet.</em></h2><p>A different retirement date. A little more spending. A smaller down payment. Ask a follow-up and see what changes in the plan—and why.</p><Link className="text-link" href="/demo">See a plan in action <ArrowRight size={16} aria-hidden="true" /></Link></div>
          <AssumptionPreview />
        </section>
      </div>
      <section className="story-trust" aria-labelledby="trust-title">
        <div className="shell story-trust-inner"><div><p className="section-kicker light">SHOW THE MATH</p><h2 id="trust-title">A clear answer.<br /><em>And the work behind it.</em></h2><p>See the inputs, assumptions, calculations, and sources behind your answer. Purpose-built tools handle supported financial math.</p><Link className="light-link" href="/trust">See how answers are checked <span aria-hidden="true">→</span></Link></div>
      ...
      <section className="story-closing shell" id="pricing" aria-labelledby="start-title"><p className="section-kicker">START WITH YOUR QUESTION</p><h2 id="start-title">Your next move<br /><em>starts here.</em></h2><p>30 days free. Then {pricing.label}.<br />One plan. Full access. Cancel anytime.</p><MarketingGetStartedButton className="button button-primary" trackingLocation="homepage_final" csOverrideId="cta-start-free-trial-pricing-premium" /><small>No credit card required.</small><Link className="text-link" href="/pricing">See pricing details <span aria-hidden="true">→</span></Link></section>
      <SiteFooter />
    </main>
  );
}
