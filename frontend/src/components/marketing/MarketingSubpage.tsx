...
function UseCasePage({ useCase }: { useCase: UseCaseKey }) {
  ...
  return (
    <StandardPage className={`use-case-page ${item.tone}`}>
      ...
      <section className="case-context dark-band"><div className="shell case-context-inner"><div><p className="section-kicker light">CONTEXT LINC BRINGS IN</p><h2>No exports. No tab juggling.</h2></div><div className="context-chips">{item.context.map((context)=><span key={context}>{context}</span>)}</div></div></section>
