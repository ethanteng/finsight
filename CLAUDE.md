...
Neither calculator shows its answer until the visitor gives an email address. The submit sends the results email and reveals the result in place, and one address unlocks the rest of the tab on both pages. This is a nudge, not a control: the figures reach the browser either way. See "The email gate" in `docs/COAST_FIRE_EMAIL_CAPTURE.md`.
