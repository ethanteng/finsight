...
flowchart LR
    ...
    E -->|"existing plan accepted"| G["Write answer"]
    F --> G
...
2. It returns strict JSON: a boolean for every allowlisted pack, whether independent reasoning review is useful, a short explanation, and retirement inputs explicitly stated by the user.
...
5. Before answering, the configured primary analysis model must call `request_data_packs`. It sees the transcript, selected pack IDs, and available canonical fact labels. It may accept the plan or request additional allowlisted packs; it cannot remove packs.
6. Requested additions are committed only after the wider data read succeeds. The same primary model then writes the answer from the final fact pack.
7. Deterministic grounding remains the last safety boundary. If an answer still reaches for evidence it did not receive, recovery is exhaustive: load all remaining allowlisted packs and re-check or regenerate. This recovery does not infer a pack from the shape of a number or from language rules.
...
Every new evidence manifest stores the initial semantic plan, final packs, primary-tool outcome, planner/tool latency, and any late recovery. Older manifests remain readable as legacy observations.
