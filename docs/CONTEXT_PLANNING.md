...
2. It returns strict JSON: a boolean for every allowlisted pack, whether independent reasoning review is useful, a short explanation, retirement inputs explicitly stated by the user, and any supported scenario policy the user wants calculated.
...
6. Requested additions are committed only after the wider data read succeeds. If a scenario was requested, application code runs its deterministic calculator and adds the results as scenario-scoped canonical facts.
