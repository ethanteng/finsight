...
5. If the baseline still stops for want of planning inputs that the requested variant is itself carrying, the pipeline completes it once more with those values folded into the fields it lacks, and only when they close every gap. Without that step the projection asks for numbers the user has already given while the scenario reports that it needs a completed baseline, and neither side can move. A baseline that can already run is never rewritten, so a genuine what-if keeps its comparison.
