...
function isNumericIdentifier(prose: string, index: number, rawValue: string): boolean {
  ...
  // "10%" never matches). Look past the usual 16-char window so
  // "of historical sequences" still fits.
  const afterDistribution = prose.slice(index + rawValue.length, index + rawValue.length + 48);
  if (
    /\b(?:worst|best|bottom|top|lowest|highest)\s+$/i.test(before) &&
    /^\s*%\s+of\s+(?:the\s+)?(?:historical\s+)?(?:sequences?|outcomes?|cases?|scenarios?|paths?|simulations?|windows?|runs?)\b/i
 ...
 */
// Every cue here must name a loss the pack stores signed. Bare "gap of",
// "down by" and "behind by" describe a difference in either direction, and
// the comparison facts publish theirs as positive magnitudes via
// abs(input[0] - input[1]) -- monthly cost, upfront cash, survival rate. Only
// gaps that name a shortfall (reserve / funding / emergency-fund) are signed.
const NEGATIVE_WORDING_BEFORE =
  /\b(?:negative|minus|deficit\s+of|shortfall\s+of|shortage\s+of|(?:reserve|funding|emergency(?:-|\s+)fund)\s+gap\s+of|short(?:falling)?\s+by|in\s+the\s+red\s+by)\s*(?:about\s+|roughly\s+|around\s+|approximately\s+|some\s+)?$/i;
