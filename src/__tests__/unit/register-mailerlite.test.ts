...
describe('registration and the marketing list', () => {
  ...
  /*
   * With nothing configured there is no group to join, and subscribing anyway
   * would add a no-group subscriber that the nightly sync was going to add
   * regardless — an API call, and a list entry, for no difference.
   */
  it('does not call MailerLite at all when no group is configured', async () => {
    for (const name of Object.keys(GROUPS)) delete process.env[name];

    const res = await register({ signupOrigin: 'retirement_calculator' });
    await settle();
