...
describe('SnapTrade Workflow Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    // Generate JWT token
    userJWT = await hashPassword('testpassword123'); // This will be the JWT in real implementation
  });
  ...
  describe('Complete SnapTrade User Workflow', () => {
    it('should handle complete SnapTrade user lifecycle', async () => {
      ...
      expect(accountsResponse.body.data).toHaveLength(1);
      expect(accountsResponse.body.data[0]).toHaveProperty('name', 'Test Investment Account');
      ...
      expect(holdingsResponse.body.data[0]).toHaveProperty('symbol', 'AAPL');
      ...
      expect(activitiesResponse.body.data).toHaveLength(1);
      expect(activitiesResponse.body.data[0]).toHaveProperty('type', 'buy');
